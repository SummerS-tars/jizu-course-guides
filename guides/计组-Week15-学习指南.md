...
| **DMA (Direct Memory Access)** | 直接内存访问：设备控制器绕过 CPU 批量搬运内存数据 |
| **周期挪用 (Cycle Stealing)** | DMA 抢用若干总线周期传数据，CPU 暂停让路 |
...
#### 2.2.1 先看问题：机械磁盘为什么随机读慢？

机械磁盘的慢不主要来自“传一个扇区要多久”，而是来自定位：磁头要先移动到目标磁道，再等待盘片转到目标扇区。随机 I/O 每次都可能重新寻道和等待旋转，因此延迟以毫秒计；顺序 I/O 可以摊薄定位开销，吞吐更高。（来源：w15-io-disk-raid）
...
| 项 | 含义 | 典型影响 |
|----|------|----------|
| 寻道时间 | 磁头移动到目标磁道 | 随机访问最大瓶颈 |
| 旋转延迟 | 等目标扇区转到磁头下方 | 平均约半圈 |
| 传输时间 | 扇区经过磁头并传出数据 | 小块随机读中通常较小 |

#### 2.2.2 示例题：7200 RPM 随机读延迟
...
> **读图提示：** 先读定位链路：寻道决定磁头到哪条轨道，旋转延迟决定目标扇区何时到达磁头下，最后才是实际传输。三段串行相加，不能把 RPM 直接当成吞吐。

| 项 | 内容 |
|----|------|
| **题目场景** | 随机读取机械硬盘上的一个扇区 |
| **已知** | 转速 7200 RPM；平均寻道 10 ms；传输 0.1 ms |
| **求** | 平均访问延迟 |
| **步骤 1** | 一分钟 7200 圈 → 每圈 $60/7200=0.00833s=8.33ms$ |
| **步骤 2** | 平均旋转延迟约半圈 → $8.33/2=4.17ms$ |
| **步骤 3** | 总延迟 $10+4.17+0.1=14.27ms$ |
| **结果解释** | 寻道 + 旋转占绝大部分，小块传输时间几乎可忽略 |

> **易错提醒：** RPM 越大，主要改善旋转延迟；若随机访问由寻道主导，单纯提高转速不能把延迟降到微秒级。SSD 没有机械寻道，延迟模型不同。

#### 2.2.3 RAID：速度、容量、容错三角权衡
...
> **直观理解：** RAID 0 把速度拉满但没有安全网；RAID 1 用容量换可靠；RAID 5/6 用校验在空间利用与容错之间折中；RAID 10 常用于既要性能又要恢复速度的场景。
...
> **本节要回答**：CPU 怎样和设备通信？中断与 DMA 分别省掉什么开销？周期挪用为什么会影响 CPI？
...
#### 2.3.1 MMIO：把设备寄存器放进地址空间

**MMIO（Memory-Mapped I/O，内存映射输入输出）** 将外设控制寄存器映射到某段物理地址。CPU 对这些地址执行 load/store，本质上是在读写设备寄存器，而不是普通 DRAM 数据。总线 **Valid/Ready** 握手与 Lab2 访存时序一脉相承；Difftest 对 MMIO 用 **PA** 并 Skip，普通访存用 **VA**。（来源：w15-study-priority）

> **边界说明：** MMIO 地址通常不应被普通 Cache 随意缓存，否则 CPU 可能读到设备寄存器旧值或把写操作延迟。开卷题看到“设备寄存器”“mtime/mtimecmp”“串口”等关键词，应先想到 MMIO。

#### 2.3.2 中断驱动 I/O：设备完成后再叫醒 CPU

轮询方式让 CPU 不断读设备状态位，简单但浪费周期；中断方式让 CPU 先去做别的事，设备准备好后触发中断，CPU 进入 ISR（Interrupt Service Routine，中断服务程序）处理。

| 方式 | CPU 做什么 | 适合场景 | 代价 |
|------|------------|----------|------|
| 轮询 | 不断检查状态寄存器 | 极短等待、实现简单 | 空转浪费 CPU |
| 中断 | 设备就绪后异步通知 | 事件少、延迟敏感 | 保存/恢复现场开销 |
| DMA | CPU 初始化后由控制器批量搬运 | 大块连续 I/O | 总线争用、Cache 协同 |

#### 2.3.3 DMA 与周期挪用：批量搬运但抢总线

**DMA（Direct Memory Access，直接内存访问）** 的流程是：CPU 配置源地址、目的地址、长度与方向；DMA 控制器直接在设备和主存之间搬运；完成后用中断通知 CPU。这样避免“每个字节/字都让 CPU load/store 一次”。

```mermaid
sequenceDiagram
    participant CPU
    participant DMA as DMA 控制器
    participant DEV as I/O 设备
...
> **读图提示：** CPU 只负责初始化和收尾；中间的数据通路由 DMA 控制器接管。周期挪用发生在 DMA 访问主存总线时，CPU 可能暂时等待，因此有效 CPI 会变差。
...
|----|------|
| **题目场景** | 从磁盘搬 4KB 数据到内存 |
| **已知** | 若逐字节中断，CPU 每字节都进入 ISR；若 DMA，只在开始配置和结束中断时介入 |
| **求** | 哪种 CPU 开销更低，代价是什么 |
| **步骤** | 逐字节中断触发约 4096 次 ISR；DMA 中间批量搬运，CPU 只处理少量控制操作 |
| **结果解释** | DMA 大幅降低 CPU 指令开销，但搬运期间会争用内存总线，并需要考虑 Cache 中脏块/旧块同步 |

> **易错提醒：** DMA 不是“完全免费”。如果设备写入主存，而 CPU Cache 里还有旧副本，软件或硬件必须做 invalidate/flush；否则 CPU 可能读到旧数据。
...
1. 估算一次随机磁盘读的平均延迟（给 RPM 与寻道时间）
2. 说出 RAID 0/1/5/10 各自的速度、容错、空间特点
3. 对比轮询、中断、DMA 的 CPU 开销与适用场景
4. 解释 DMA 周期挪用如何影响有效 CPI，以及为什么要处理 Cache 协同
5. 画出 Week 10→14 四步存储链并说明每步解决什么
6. 列出期末极高优先级主题及对应 Lab
7. 解释 Lab5 中 satp 变更后为何必须 flush 流水线
...
> **追问 4**：DMA 把设备数据写入主存后，CPU 立即读取同一缓冲区却看到旧值，可能是什么原因？
>
> **答**：CPU 的 Cache 里可能仍有该缓冲区的旧副本。DMA 写主存不一定自动更新私有 Cache，软件/硬件需在 DMA 前后做 cache clean/invalidate 或使用不可缓存映射，保证 CPU 读到设备写入的新数据。
