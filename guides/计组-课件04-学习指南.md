...
先看一张简化布局图。图中每行按从高位 bit[31] 到低位 bit[0] 排列，重点看 `rs1/rs2/rd/opcode` 的固定位置，而不是把每个 bit 宽画到比例尺一样。

```mermaid
flowchart TB
  R["R 型 Register：funct7 | rs2 | rs1 | funct3 | rd | opcode"]
  I["I 型 Immediate：imm[11:0] | rs1 | funct3 | rd | opcode"]
  S["S 型 Store：imm[11:5] | rs2 | rs1 | funct3 | imm[4:0] | opcode"]
  B["B 型 Branch：imm[12,10:5] | rs2 | rs1 | funct3 | imm[4:1,11] | opcode"]
  U["U 型 Upper：imm[31:12] | rd | opcode"]
  J["J 型 Jump：imm[20,10:1,11,19:12] | rd | opcode"]
```

> **读图提示：** 先纵向对齐 `opcode`、`rd`、`rs1`、`rs2`。S 型没有 rd，是因为 store 不写寄存器，于是把 bit[11:7] 用来放 imm[4:0]；B 型也不写 rd，于是复用这块空间放分支偏移的一部分。J 型没有源寄存器，只保留 rd 用于写返回地址。
