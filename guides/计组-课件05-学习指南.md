...
| 阶段缩写 | 英文全称 | 中文名 | 动作 |
|----------|----------|--------|------|
| IF | Instruction Fetch | 取指 | 按 PC 读指令存储器，计算 PC+4 |
| ID | Instruction Decode | 译码 / 读寄存器 | 译码产生控制信号，读寄存器堆 |
| EX | Execute | 执行 | ALU 运算或算有效地址 |
| MEM | Memory Access | 访存 | Load/Store 读写数据存储器；其他指令空操作 |
| WB | Write Back | 写回 | 结果写回目的寄存器 |
...
下面的图要解决“R 型、Load、Branch 三类指令在单周期通路里分别走哪些硬件”。Reg 表示 Register File（寄存器堆），DM 表示 Data Memory（数据存储器），Zero 是 ALU 比较结果标志。
...
> **读图提示：** 先沿 R 型看“寄存器→ALU→寄存器”的最短写回路；再沿 Load 看多出的 DM 读存储器，因此常决定单周期时钟；最后看 Branch，它主要用 ALU 的 Zero 改 PC，不写回通用寄存器。
...
> **易错提醒：** 控制信号题若来自课件 05，先按 MIPS 表理解；若来自 Lab 或 RISC-V 指令编码，优先按实验实现和 Week1-3 的 RISC-V 数据通路口径。
...
> **读图提示：** 主控先根据 opcode 判断大类，生成写使能、MUX 选择和粗粒度 ALUop；只有 R 型等需要进一步区分具体 ALU 运算时，局部控制器再看 func/funct 字段。
