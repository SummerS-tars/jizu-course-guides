...
> **边界说明：** 课件 05 的单周期数据通路多以 **MIPS 单周期 CPU** 为教学模型，因此会出现 `rs/rt/rd`、`RegDst`、`MemtoReg`、`Branch`、`Jump` 等 MIPS 教材常用字段和控制信号。本课程 Lab 主线是 **RISC-V / 26-Arch**，字段名、立即数生成和部分控制信号不同；这些图主要用于理解“数据如何从 RF → ALU → DM → RF / PC 如何更新”的通用结构，不应逐字等同到 RISC-V RTL。
...
> **边界说明：** 这里沿用 MIPS 字段名读课件图：`rs`（source register）是第一个源寄存器；`rt`（target/source register）在 MIPS 中语义随指令变，R 型里是第二源，`lw` 里是写回目标，`sw` 里是写入内存的数据源；`rd`（destination register）是 R 型目的寄存器。与 RISC-V 粗略对照：`rs ≈ rs1`，`rt` 在 R 型 / store 中像 `rs2`、在 load 中像 `rd`，`rd ≈ rd`；复习 Lab RTL 时以 RISC-V `rs1/rs2/rd` 和立即数格式为准。
...
图中三条路径分别回答三类高频题；其中分支和跳转都归到“决定下一条 PC”的路径，只是分支要判断条件，跳转直接给目标地址。

| 指令类型（数据流类型） | 怎么理解 | 核心数据流 | 关键区别 | 关键控制 |
|------------------------|----------|------------|----------|----------|
| **R 型运算** | 两个寄存器送 ALU，结果写回寄存器。 | RF → ALU → RF | 不访问 DM；写回 ALU 结果 | RegWrite=1, ALUSrc=0, MemtoReg=0 |
| **Load** | 先算内存地址，再从 DM 读数据写回 RF。 | RF + imm → ALU → DM → RF | 多走一次 DM 读；写回内存数据 | RegWrite=1, ALUSrc=1, MemtoReg=1 |
| **Branch/Jump** | 不关心写普通数据，重点是判断或计算下一条 PC。 | RF → ALU 比较 / 目标地址 → PC | 通常不写 RF/DM；只可能改 PC | Branch 结合 Zero；Jump 直接选择跳转目标 |
...
> **边界说明：** 本课件真值表以 MIPS 风格信号讲解：`RegDst` 处理 MIPS `rt/rd` 写回目的寄存器选择，`MemtoReg` 处理 ALU/DM 写回数据选择，`Branch`/`Jump` 对应 MIPS `beq/j` 的 PC 选择。本课程 Lab 主线是 RISC-V / 26-Arch，字段位置、跳转指令和部分控制名会不同，但思路相同：先确定写回、访存、ALU 操作数、PC 来源，再落到实验 RTL 的实际信号。Lab 细节见 `计组-Lab1-6-整合指南.md` 的 Lab1–3。（来源：kejian05-partB-control、guides/计组-Lab1-6-整合指南.md）
...
> **边界说明：** 这张状态序列表仍是 MIPS 风格多周期处理器示例（`lw/sw/R/beq/j`）。RISC-V 实验会把 load/store、branch、`jal/jalr`、CSR/trap 等按自己的译码与流水线控制实现；这里主要借它理解“公共取指/译码 + 按指令类型分流”的控制组织。
