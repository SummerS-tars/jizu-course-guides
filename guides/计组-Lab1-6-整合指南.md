...
**SATP 注意点**：报告与 NotebookLM raw 都强调 SATP 是 Week11 高频点。当前 GitHub 代码中 `CSR_SATP` 可读写并导出给 MMU；但 `satp` 的 WARL 合法化细节在代码片段中表现为直接写入，是否另有约束需待核对。复习时要把“实验能跑”与“期末问 WARL 机制”分开理解。
