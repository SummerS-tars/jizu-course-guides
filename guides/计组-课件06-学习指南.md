...
1. **先画五级时空图**：横轴周期、纵轴指令，先标 IF（取指）/ID（译码）/EX（执行）/MEM（访存）/WB（写回），再判断同周期谁需要同一资源或同一寄存器值。
...
其中 IC（Instruction Count，指令条数）来自程序；CPI（Cycles Per Instruction，每指令周期数）由理想流水和冒险惩罚共同决定；$T_{clock}$ 是一个时钟周期长度。

**示例题：Load-Use 对 CPI 的影响**

**题目场景**：理想五级流水每周期完成一条指令，但部分 load 后紧跟使用者，需要插入气泡。

**已知**：理想 CPI=1；程序 20% 为 load；Load-Use 后继使用概率 25%；每次罚 1 周期。

**求**：额外 CPI 与实际 CPI。

**公式**：额外 CPI = 触发指令比例 × 触发概率 × 单次惩罚周期。

**步骤**：额外 CPI = $0.2 \times 0.25 \times 1 = 0.05$；实际 CPI = $1 + 0.05 = 1.05$。

**结果解释**：Load-Use 只发生在一部分 load 后面，因此不是所有 load 都加 1 拍；平均到每条指令后只增加 0.05 CPI。（首轮监修补强，本轮补齐题面）

> **易错提醒：** 题目若给的是“20% 指令为 load，且其中 25% 触发”，要相乘；不要把 25% 当成全程序比例直接加。
...
> **本节要回答**：IF（Instruction Fetch，取指）/ID（Instruction Decode，译码）/EX（Execute，执行）/MEM（Memory Access，访存）/WB（Write Back，写回）各做什么？控制信号如何传递？

### 块 B.1 五阶段

| 阶段缩写 | 英文全称 | 中文名 | 功能 |
|----------|----------|--------|------|
| **IF** | Instruction Fetch | 取指 | 按 PC 取指，PC+4 |
| **ID** | Instruction Decode | 译码 / 读寄存器 | 译码，读寄存器，立即数扩展 |
| **EX** | Execute | 执行 | ALU 运算或算址 |
| **MEM** | Memory Access | 访存 | Load/Store 访存 |
| **WB** | Write Back | 写回 | 写回寄存器 |

**段间寄存器**（IF/ID、ID/EX、EX/MEM、MEM/WB）保存段间状态。（来源：kejian06-partB-datapath）

### 块 B.2 控制信号传递

下面的图要解决“为什么 ID 阶段产生的控制信号必须随指令一路向后带”。ALUSrc/ALUOp、Branch/MemWr、RegWr/MemtoReg 分别在 EX/MEM/WB 阶段才真正使用。
...
> **读图提示：** 不要把控制信号理解成“全局一直有效”。它们和对应指令一起锁存在段间寄存器中，到了相应阶段才控制 MUX、写使能或分支选择；Flush 本质上就是把错误路径指令携带的控制信号清零。
...
转发要解决的是“生产者结果已经在流水线后段出现，但还没写回 RF（Register File，寄存器堆）”。检测时先看最近的 EX/MEM，再看更远的 MEM/WB。
...
> **边界说明：** `lw` 的数据到 MEM 末才稳定，而紧随其后的使用者在下一周期 EX 开始就需要操作数。转发只能把“已经产生”的结果旁路过来，不能让尚未从数据存储器读出的值提前出现。

### 块 C.4 时空图示例

**示例题：转发 + Load-Use 阻塞**

**题目场景**：给定连续指令，判断哪些 RAW（Read After Write，写后读）可转发，哪里必须插入气泡。
...
| `and r7,r2,r8` |  |  |  | IF | ID | **Stall** | EX | MEM | WB |
| `or r9,r7,r10` |  |  |  |  | IF | **Stall** | ID | EX | MEM |

**步骤**：

1. `add → sub` 是 R 型 RAW；`add` 的 ALU 结果在 EX/MEM 可用，`sub` 的 EX 阶段可从 EX/MEM 转发。
2. `lw → and` 是 Load-Use；`lw` 数据到 C6 的 MEM 末才可用，而 `and` 原本 C6 就要进 EX，必须插入 1 个气泡。
3. `and → or` 在阻塞后距离被拉开，`or` 可在 C8 EX 从 `and` 的 EX/MEM 或 MEM/WB 旁路取值。

**结果解释**：转发减少等待，但 Load-Use 至少要 1 拍；Stall 冻结 PC/IF/ID，让后续 `or` 也被动后移。

> **易错提醒：** 表中 `or` 的 Stall 不是因为它自己需要 `lw`，而是前面的 `and` 冻结在 ID，后续取指/译码一起停住。
...
**示例题：EX 决断时的 Flush 范围**

**题目场景**：RISC-V 条件分支在 EX 阶段比较并生成目标 PC，前端已经按顺序取了若干条后继指令。

**已知**：分支实际跳转；预测为不跳；决断点在 EX。

**求**：为什么要 Flush，冲刷哪些阶段。

**步骤**：

1. 分支进入 EX 前，IF/ID 中已经装入顺序路径指令。
2. EX 发现应跳转后，PC 改为分支目标。
3. 顺序路径上已取/译的年轻指令不是正确路径，必须把对应段间寄存器控制信号清零。

**结果解释**：Flush 是“丢掉错误路径”，不是“等待数据”；它与 Stall 的硬件动作和语义不同。

> **易错提醒：** 分支越早决断，错误路径越短，但 ID 决断可能需要更早的比较器、转发和目标地址计算硬件。
