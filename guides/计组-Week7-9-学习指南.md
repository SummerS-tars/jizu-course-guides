...
3. 区分结构/数据/控制三类冒险，并说明 Stall 与 Flush 的不同语义
4. 对 `lw` 后接消费者的指令序列画出 1 拍阻塞时空图
5. 判断 ALU→ALU RAW 何时可由 EX/MEM 或 MEM/WB 转发解决
6. 对比 Scoreboard 与 Tomasulo 对 WAR/WAW 的处理
7. 填写 Tomasulo 中 Qi 与 RS 的 Qj/Qk，并解释 CDB 与 ROB 的分工
8. 对照 Lab1/Lab3 说出转发 MUX 与 Flush 的触发条件
...
> **追问 3**：Tomasulo 中一条乘法结果上 CDB 时，哪些保留站会同时更新？为何仍需 ROB？
>
> **答**：所有 Qj 或 Qk 指向该乘法 RS 的保留站会**同时**捕获结果。ROB 保证**按序提交**与**精确异常**——乱序执行的结果不能随意写回架构寄存器/内存。

> **追问 4**：Lab3 若在 MEM 而非 EX 判定分支，控制冒险代价如何变化？
>
