...
下面的图要解决“监听法和目录法到底谁给谁发消息”。Snoop/Snoopy 指监听式广播，Directory 指目录式点对点维护持有者集合。
...
> **读图提示：** 左侧所有 Cache 都挂在共享总线上，因此每次关键事务都广播；右侧目录保存“哪些核心持有该块”，只向相关核心发点对点消息。小规模时广播简单，核数增多后目录能避免无关核心也被迫监听。
...
下面的状态图解决“本地读写和他人总线事件分别如何改变状态”。PrRd/PrWr 表示本核心处理器读/写；BusRd/BusRdX/Invalidate 表示从总线上监听到他人的读、独占读或作废请求。
...
> **读图提示：** 先按事件来源分两类：本地 PrRd/PrWr 往往让本行获得或升级权限；他人 BusRd/BusRdX 往往让本行降级或失效。M 态遇到他人读要 Flush，因为主存旧了；E 态本地写可直接变 M，因为没有其他副本。

（来源：kejian08-partB-mesi、[Week13-14 指南](计组-Week13-14-学习指南.md) §2.4）

### 块 2.3 MESI 数值模拟示例

**题目场景**：两个核心 P1/P2 读写同一地址 A，用 MESI 状态表判断每一步是否需要总线事务。

**已知**：地址 A 初值 5，Cache 采用 MESI，P1/P2 中 A 的初始状态全为 I。

**求**：每一步后 P1/P2 状态、总线事务和数据来源。
...
**结果解释**：第 4 步 P2 能读到 10，是因为 P1 的 M 态副本被 BusRd 触发 Flush；若没有一致性协议，P2 可能只看到主存旧值 5。

> **易错提醒：** 表中 BusRdX/BusUpgr 都服务于“我要写并获得独占权限”，但 BusRdX 常用于本地没有有效副本时读独占，BusUpgr 常用于已有 S 副本时只升级权限。
...
**示例题 A：R1=0, R2=0 是否可能**

**题目场景**：两个核心各自写一个变量，再读对方变量。

**已知**：P1：`A=1; R1=B;`；P2：`B=1; R2=A;`；初值 A=B=0。

**求**：弱模型下是否可能 R1=0 且 R2=0？SC 下是否可能？

**步骤**：

1. SC 要求每个处理器的程序序保持，同时所有访存能排成一个全局序。
2. 若 R1=0，说明 P2 的 `B=1` 尚未对 P1 可见；若 R2=0，说明 P1 的 `A=1` 尚未对 P2 可见。
3. 写缓冲 + 提前读可让两个本地读先于对方写可见，于是弱模型下可能同时读到 0。

**结果解释**：R1=0 且 R2=0 在 SC 下不可能，在弱模型下可能；这是 Consistency 问题，不是同地址 Coherence 失效。
...
> **易错提醒：** 旗语反例里 `Flag` 和 `Data` 是不同地址；即使每个地址自身都满足 Coherence，消费者仍可能因为跨地址可见顺序错误读到旧数据。
...
下面的图要解决“Fence 在生产者/消费者同步里约束哪一段顺序”。它不是 MESI 状态图，而是跨地址访存顺序图。
...
> **读图提示：** 生产者侧的 fence/release 保证 `Data=NEW` 先于 `Flag=Ready` 对外可见；消费者侧 acquire 保证看到 Ready 后再读 Data。若题目讨论页表 PTE 或 `satp`，不要套这张普通数据图，应回到 `SFENCE.VMA`。
